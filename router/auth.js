...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res.status(500).send(new Error("createuser error", "Internal Server Error"));
    }
  }
)
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res.status(500).send(new Error("login error", "Internal Server Error"));
    }
  }
)


// ROUTE 3: Get logged in user data: GET "/api/auth/getuser", Login Required
router.get(
  "/getuser",
  ...
  async (req, res) => {
    try {
      ...
      res.status(500).send(new Error("getuser error", "Internal Server Error"));
    }
  }
)
